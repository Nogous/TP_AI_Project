...
namespace GoldSquadron
{
    ...
    public class GoldSquadronMovementSystem : MonoBehaviour
    {
        [Header("Heuristique")]
        [SerializeField]
        ...
        public void InitializeSystem(SpaceShip spaceship, GameData newData)
        {
            ship = spaceship;
            data = newData;
        ...
        public WaypointData SelectBestWaypoint(Vector3 start)
        {
            ...
            if(pointsFront.Count == 0 || (bestDistanceFront > 20 && pointsBehind.Count > pointsFront.Count))
            {
        ...
        public bool MoveToPoint()
        {
            ...
            if ((ship.Velocity.magnitude >= (ship.SpeedMax * 0.1f)) && (Vector2.Angle(ship.Velocity, currentWayPoint.Position - ship.transform.position) < goodAngleForPreshot) && (distance < distancePreshot)) // On prend de l'avance
            {
                if (nextWayPoint == null)
                {
                    nextWayPoint = SelectBestWaypoint(currentWayPoint.Position);
